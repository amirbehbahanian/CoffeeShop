...
class testBE(unittest.TestCase):
    ...
    def test_barista(self):
        barista = self.container.barista_factory(
            csv_file=csv_path, level=Skill.midlevel
        )
    ...
    def test_rush_hour(self):
        ...
        rush_hour.find_barista_and_order(
            time=datetime(2024, 10, 10, 12, 10), logger=logger
        )
        self.assertEqual(waiting_line.count_customers(), 2)
        self.assertEqual(
            rush_hour.barista_list[0].customer.order_start_time,
            datetime(2024, 10, 10, 12, 10),
        )
        self.assertEqual(rush_hour.barista_list[0].customer.status, Status.ordering)
        rush_hour.find_barista_and_order(
            time=datetime(2024, 10, 10, 12, 10, 50), logger=logger
        )
        self.assertEqual(rush_hour.drink_wait_list[0].status, Status.waiting_for_drink)
        self.assertEqual(
            rush_hour.drink_wait_list[0].order_time, datetime(2024, 10, 10, 12, 10, 50)
        )
        self.assertIsInstance(rush_hour.drink_wait_list[0].order, Drink)
        save_path = os.path.join(os.getcwd(), "my_logger.out")
        if os.path.exists(save_path):
            os.remove(save_path)
        rush_hour.serve_drink_wait_list(
            time=datetime(2024, 10, 10, 12, 25, 50), logger=logger
        )
    ...
    def test_start_consuming(self, mock_blocking_connection):
        ...
        mock_channel.basic_get.return_value = (
            mock_method_frame,
            None,
            json.dumps({"barista": 2, "customer": 5}).encode(),
        )

        simulation = Simulation(menu_path="test.csv")

        with patch("builtins.print") as mocked_print:
            simulation.consuming_test(
                max_iterations=1
            )  # mock the print and set max iteration to 1
        ...
        mock_connection.channel.assert_called_once()
        mock_channel.queue_declare.assert_called_once_with(
            queue="run_simulation", durable=True
        )
